...
public class TireEditor extends Container {
    ...
    private static PropertyPanel createPropertyPanel(TireSettings tireSettings) {
        ...
        result.addFloatProperty("SlipAngle C", tireSettings,
                "slipAngleCoefficientC", 0, 5, 0.001f);

        // affects how "tight" the curve is.
        result.addFloatProperty("SlipAngle B", tireSettings,
                "slipAngleCoefficientB", 0, 30, 0.001f);

        // affects how much the slip "drops off".0...
        result.addFloatProperty("SlipAngle E", tireSettings,
                "slipAngleCoefficientE", -2, 2, 0.001f);

        // load / force
        // I'm not certain of the difference between these two...
        // affects how much load can be applied.
        result.addFloatProperty(
                "Load KA", tireSettings, "loadCoefficientKA", 0, 10, 0.001f);

        // affects how much load can be applied to the tire.
        result.addFloatProperty("Load KB", tireSettings, "loadCoefficientKB", 0,
                0.0005f, 0.000001f);
